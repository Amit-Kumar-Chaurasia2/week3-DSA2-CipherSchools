...
private:
    int Distance(int i, int j , string &word1, string &word2){
        //Two base cases 
        if(i < 0){//if first string runs out of index that means we need to insert remaining characters in string2 
            return j+1;
        }
        if(j < 0){//if second string runs out of index that means we need to delete the remaining characters of string1 to make it string2.This will add up in the operations
            return i+1;
        }
        if(word1[i] == word2[j]){
...
int minDistance(string word1, string word2) {
        ...
        for(int i = 1;i<= n;i++){
            for(int j = 1;j <= m;j++){
                if(word1[i-1] == word2[j-1]){//as 1 based indexing so shift occurs
                    dp[i][j] = dp[i-1][j-1];
                }
                else{
